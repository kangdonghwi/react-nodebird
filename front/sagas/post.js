...
function* removePost(action) {
  try {
    const result = yield call(removePostAPI, action.data); //fork는 비동기 함수호출 call은 동기 함수호출
...
function* addComment(action) {
  try {
    const result = yield call(addCommentAPI, action.data); //fork는 비동기 함수호출 call은 동기 함수호출
    yield delay(1000);
    yield put({
...
function* watchUploadImages() {
  yield takeLatest(UPLOAD_IMAGES_REQUEST, uploadImages); //takeLatest - 실수로 여러번 클릭해도 마지막것만 실행됌. 근데 앞에 이미 실행 완료된 것은 이미 실행 되서 놔두고 로딩중인것만 취소함.
}
